...
bool setMaxPower(int which) 
{
  ...
  {    // and calculate the validation byte
        consoleOut("preparing the throttle command for DS3");
        // the user inputted a value in maxPower, this should be callibrated
        int calibrated = desiredThrottle[which] + Inv_Prop[which].calib;
        Scaled = calibrated * 16.59;
        //Scaled = Inv_Prop[which].maxPower +  Inv_Prop[which].invCal * 16.59;
        ...
        snprintf(sendCommand, sizeof(sendCommand), "2401%s1414060001000F13%sFBFB06AA270000%02X%02X01%02XFEFE", Inv_Prop[which].invID, ecu_id_reverse, msb, lsb, vv);
  ...
  }  else
  {
    ...
    int calibrated = desiredThrottle[which] + Inv_Prop[which].calib;
    Scaled = calibrated * 28.89; //
    //Scaled = Inv_Prop[which].maxPower + Inv_Prop[which].calib * 28.89; // V
      ...
      //construct command
      sendCommand[0]='\0'; // make sure its empty
      snprintf(sendCommand, sizeof(sendCommand), "2401%s1414060001000F13%sFBFB061C8C02%s00", Inv_Prop[which].invID, ecu_id_reverse, hexStr);
      consoleOut("The raw sendCommand for DS3  = " + String(sendCommand));
      ...
      //append to sendCmd
      strncat(sendCommand, hexString,  sizeof(sendCommand) - strlen(sendCommand) - 1);
      strncat(sendCommand, "FEFE",  sizeof(sendCommand) - strlen(sendCommand) - 1);
      consoleOut("The raw sendCommand for YC600 / QS1 = " + String(sendCommand));
      // we have the command ready
  }
    ...
    snprintf(sendCommand, sizeof(sendCommand), "2401%s1414060001000F13%sFBFB06DE000000000000E4FEFE", Inv_Prop[which].invID, ecu_id_reverse);
